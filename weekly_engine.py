...
def evaluate_active(sheet, per_symbol, all_dates):
    ...
    for r in records:
        ...
        # close, so the two display columns are populated even on a run where
        # no NEW trading day has arrived yet (e.g. right after adding them).
        if last_eval in h.index:
            close0 = float(h.loc[last_eval, "close"])
            r["last_price"] = round(close0, 2)
            r["pnl_pct"] = round((close0 - current_price) / current_price * 100, 2)
        ...
        for day_idx in range(start_idx, len(all_dates)):
            ...
            # independent of the exit-ladder's own day-count decision below.
            r["last_price"] = round(close, 2)
            r["pnl_pct"] = round(ret_pct, 2)
