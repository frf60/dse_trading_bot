...
from config import TOP_N_DAILY, MIN_BARS_REQUIRED, MIN_SCORE, SCORE_MAX, SELECTIVE_SIGNAL_ENABLED
...
from selective_signal import run_selective_signal_pipeline
...
def main():
    ...
    # 4. Selective Signal engine — fully independent 3rd system (own tabs:
    #    SS_Candidates / SS_Active / SS_Sell). Runs after the trading engine
    #    since order between them doesn't matter, but must run after
    #    ingest_staging() above so it sees today's row.
    if SELECTIVE_SIGNAL_ENABLED:
        run_selective_signal_pipeline(sheet, run_date)
