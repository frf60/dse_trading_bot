...
Open is now tracked too (added for the Selective Signal engine, which
...
RAW_HEADER = ["date", "ticker", "open", "high", "low", "close", "volume"]
...
def _match_columns(header: list) -> dict:
    ...
    "open" was added for the Selective Signal engine, which fills entries
    at the next trading day's open — AmarStock's CSV export already has an
    Open/OPENP column, so this is new wiring here, not a new data source.
    """
...
def ingest_local_backfill(sheet, path: str = "data/amarstock_backfill.csv") -> dict:
    ...
    for _, row in df.iterrows():
        ...
        if key in existing_keys:
            continue
        new_rows.append([row["date"], row["ticker"], row["open"], row["high"], row["low"], row["close"], row["volume"]])
...
def get_historical_data(sheet, ticker: str, days: int = 100) -> pd.DataFrame:
    """
    Columns: open, high, low, close, volume — indexed by date, oldest ->
    newest. 'open' was added for the Selective Signal engine, which fills
    entries at the next trading day's open; every other indicator in this
    project still only needs high/low/close/volume.
    """
